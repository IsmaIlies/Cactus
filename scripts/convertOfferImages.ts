...
async function convert() {
  ...
  for (const file of targets) {
    ...
    // Produce a nice kebab-case name
    // Map some common variants to stable slugs
    ...
    const simple = slugify(base.replace(/%/g, '')); // rough fallback
    const key = base
      .toLowerCase()
      .replace(/%/g, '')
      .normalize('NFD')
    ...
    const targetName = mapping[key] || simple;
