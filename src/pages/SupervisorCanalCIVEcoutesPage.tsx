...
import { collection, doc, onSnapshot, query, Timestamp, where, writeBatch } from "firebase/firestore";
...
type EcouteRow = {
  ...
  saleStatusRaw?: string;
  isTAValidated?: boolean;
};
...
const SupervisorCanalCIVEcoutesPage: React.FC = () => {
  ...
  React.useEffect(() => {
    setLoading(true);
    setError(null);
    // CIV-only data: query strictly on region, sort client-side by date/startedAt desc
    const q = query(
      collection(db, "sales"),
      where("region", "==", 'CIV')
    );
    ...
    const unsubscribe = onSnapshot(
      q,
      (snapshot) => {
        const next: EcouteRow[] = [];
        snapshot.docs.forEach((d) => {
          const data = d.data() as any;
          const ts: Timestamp | Date | number | null = data?.date ?? data?.createdAt ?? null;
          const startedAt = toDate(ts);
          const telephone: string | null = data?.telephone ?? data?.clientPhone ?? null;
          if (!telephone) return;
          const agentLabel: string = data?.name || data?.userName || data?.agent || data?.createdBy?.displayName || data?.createdBy?.email || "—";
          // TA validation based on sale status fields
          const saleStatusRaw: string = data?.basketStatus || data?.basketStatut || data?.status || data?.statut || '';
          const normalize = (str:string) => str
            ...
            .replace(/-/g, '')
            .replace(/[^a-z0-9]/g, '');
          const normSaleStatus = normalize(saleStatusRaw);
          const isTAValidated = normSaleStatus === 'ok' || normSaleStatus === 'validsoft';
          next.push({
            id: d.id,
            startedAt,
            telephone,
        ...
        next.forEach((row) => { if (row.ecouteStatus === "validated") seed[row.id] = true; });
        setStatuses(seed);
        setLoading(false);
      },
      (err) => {
  ...
  const agentOptions = React.useMemo(() => {
    ...
    rows.forEach((row) => { if (row.agentLabel) set.add(row.agentLabel); });
  ...
  const filteredRows = React.useMemo(() => {
    return rows.filter((row) => {
      ...
      // Afficher uniquement les ventes validées par le TA
      if (!row.isTAValidated) return false;
      return true;
    });
  ...
  const persistStatuses = React.useCallback(async () => {
    ...
    setSaveMessage(null);
    try {
      // Only update documents whose state actually changed
      const changes = rows.filter((row) => {
        const want = !!statuses[row.id];
        const isValidated = row.ecouteStatus === "validated";
        return want !== isValidated;
      });

      if (changes.length === 0) {
        setSaveMessage("Aucun changement à sauvegarder.");
        return;
      }

      // Firestore limit: 500 operations per batch. Chunk to be safe.
      const chunkSize = 450;
      for (let i = 0; i < changes.length; i += chunkSize) {
        ...
        chunk.forEach((row) => {
          ...
          batch.update(doc(db, "sales", row.id), { ecouteStatus: isValidated ? "validated" : null });
        });
        await batch.commit();
      }
      setSaveMessage("Statuts sauvegardés.");
    } catch (err: any) {
      const msg = String(err?.message || "Impossible de sauvegarder les statuts.");
      setSaveMessage(msg);
    } finally {
      setSaving(false);
  ...
  const statusLabel = (status: boolean | undefined) => (status ? "Validé" : "—");
      ...
      <header className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl font-semibold">N° Écoutes — CANAL+ CIV</h1>
            <p className="text-sm text-blue-200/80">Retrouvez les numéros clients saisis et validez-les.</p>
          </div>
          <button
            type="button"
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {(selectedDate || agentFilter !== "all") && (
            <div className="flex items-end">
              <button
                type="button"
                onClick={() => { setSelectedDate(""); setAgentFilter("all"); }}
...
export default SupervisorCanalCIVEcoutesPage;
